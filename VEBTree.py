...
class VebTree:
    ...
    def High(self, u, x):  # Returns a key according to which cluster is to be chosen
        return x // (int(math.sqrt(u)))

    def Low(self, u, x):  # Returns a value which is to be inserted in corresponding cluster
        return x % (int(math.sqrt(u)))
    ...
    def Insert(self, universe, x):
        if universe.u <= x or x < 0:
            ...
            return
        if universe.min == None:  # Node has no child ie. Empty node
            self.EmptyInsert(universe, x)  # Filling the min,max value
        else:
            if universe.min > x:  # Line 29-32
                universe.min, x = x, universe.min  # Updating the minimum or maximum
            elif universe.max < x:
                universe.max, x = x, universe.max

            if universe.u > 2 and x != universe.min and x != universe.max:  # For other then leaf nodes
                ...
                if universe.cluster[high] == None:
                    ...
                    self.Insert(universe.summary, high)  # to insert summary of clusters
                else:
    ...
    def Successor(self, universe, x):
        ...
        if universe.min > x:
            ...
            if universe.cluster[high] != None:  # line 77-84, To find successor in its own child-cluster
                maxinsubcluster = self.Maximum(universe.cluster[high])
            if maxinsubcluster != None and maxinsubcluster > low:  # to check whether Successor can be present in sub-cluster(low=new x)
                new_suc = self.Successor(universe.cluster[high], low)  # recursion to find successor in sub-cluster
                if new_suc != None:
                    return self.Index(universe.u, new_suc, high)  # To get the original no.
                else:
                    return None
            else:  # line 85-94, To find successor of cluster
    ...
    def Predecessor(self, universe, x):
        if universe == None or x >= universe.u or universe.min == None:    # base case
            return None
        if x<universe.min:        #in case of value of x is less than the minimum value in the tree
            return None
        if universe.max < x:       #in case of x is greator than the max in tree
            return universe.max
        elif universe.min < x:
            high = self.High(universe.u, x)
            low = self.Low(universe.u, x)
            if universe.u == 2:
                if universe.max == 1 and universe.min == 0:
                    ...
                    return None
            mininsubcluster = None   #intialize the value of minimum subcluster is None then find it
            if universe.cluster[high] != None:  # line 107-114 to find predecessor in sub-cluster
            ...
            if mininsubcluster != None and low > mininsubcluster:  # to check whether predecessor can be present in sub-cluster
                ...
                if new_pred != None:  # if  predecessors present
                    return self.Index(universe.u, new_pred, high)  # to get the original no.
                else:
                    return None
            else:  # line 115-124 To find the Predecessor of cluster cluster
                pred_cluster = self.Predecessor(universe.summary,high)  # to search predecessor of sub-cluster in summary
                if pred_cluster == None:  # if predecessor not found
                    if universe.min != None and x > universe.min:  # *In case: only min and max are present
                        return universe.min
                    else:
                        return None  # otherwise no prede.
                else:
                    new_pred = self.Maximum(
                        universe.cluster[pred_cluster])  # predecessor=maximum of the predecessor of cluster
                    return self.Index(universe.u, new_pred, pred_cluster)  # to get the original value and return it
        else:
            return None

    def Delete(self, universe, x):
        if universe == None or universe.min == None:  # universe is empty
            return False
        if x < universe.min or x > universe.max:  # x not present
            return False
        if x == universe.min:
            if x == universe.max:  # only one element is present
                universe.min = None
                universe.max = None
                return -1  # to update the summary
            if universe.summary == None:  # nothing present other then min and max
                universe.min = universe.max  # min = max and don't need to update the summary
            else:  # recursively delete the successor
                y = self.Successor(universe, x)
                high = self.High(universe.u, y)
                low = self.Low(universe.u, y)
                universe.min = y
                delvalue = self.Delete(universe.cluster[high], low)  # recursion
                if delvalue == -1:  # updating summary
                    if universe.cluster[high].min == None:  # making cluster as none
                        universe.cluster[high] = None
                    if universe.summary.min == universe.summary.max:  # only one cluster is present, cluster,summary as none
                        universe.summary = None
                    else:
                        return self.Delete(universe.summary, high)  # other cluster are present
        elif x == universe.max:
            if x == universe.min:
                universe.min = None
                universe.max = None
        ...
        else:  # to go to the cluster where element is present
            high = self.High(universe.u, x)
            low = self.Low(universe.u, x)
            if universe.cluster[high] == None:
                return False
...
k = 0
print("enter the maximum value you can insert:")
tempSize = int(input())
if tempSize == 1 or tempSize == 0:
    x = 2
else:
...
while (k != 1):
    ...
    if choose == 1:
        ...
        n = int(input())
        if n>tempSize:
            print("You can insert maximum ",tempSize+1," elements")
            n=tempSize+1
        for j in range(n):
            print("Enter element (Do not enter duplicate values)" + str(j))
            num = int(input())
            if tempSize<num:
                print("ERROR!! OUT OF RANGE")
            else:
...
for i in range(1000000):
    vt.Insert(vt.root, i)
print(vt.Predecessor(vt.root, 0))
for i in range(256*256):
    vt.Successor(vt.root, i)
    vt.Predecessor(vt.root, i)
for i in range(1000000):
    vt.Delete(vt.root, i)'''
